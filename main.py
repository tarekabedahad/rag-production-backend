...
async def rag_ingest_pdf(ctx: inngest.Context):
    def _load(ctx: inngest.Context) -> RAGChunkAndSrc:
        ...
        source_id = ctx.data.get("source_id", pdf_path)
        chunks = load_and_chunk_pdf(pdf_path)
        return RAGChunkAndSrc(chunks=chunks, source_id=source_id)

    def _upsert(chunks_and_src: RAGChunkAndSrc) -> RAGUpsterResult:
        ...
        return RAGUpsterResult(ingested=len(chunks))

    chunks_and_src = await ctx.step.run("load-and-chunk", lambda: _load(ctx), output_type=RAGChunkAndSrc)
    ingested = await ctx.step.run("embed-and-upsert", lambda: _upsert(chunks_and_src), output_type=RAGUpsterResult)
...
async def rag_query_pdf_ai(ctx: inngest.Context):
    ...
    user_content = (
        ...
        f"Context:\n{context_block}\n\n"
        f"Question: {question}\n\n"
        "Answer consicely using context above"
    )
